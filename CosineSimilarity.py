...
from treeSitter import get_function_name, extract_variables_names, modify_function_name
...
def calculate_cosine_similarity(reference, candidate):
    ...
    reference_tokens = format_tokens(hp.get_token_deltas(reference.lower()))
...
def save_to_excel(data, file_name):
    columns = [
        ...
        "Cosine Score Variables names to Reduced Code",
    ]
        ...
        for i in range(10):
            if i >= 3:
                ...
                data = [
                    ...
                    program,
                    summary,
                    get_function_name(method_body),
                    extract_variables_names(method_body),
                    calculate_cosine_similarity(summary, reduced_tokens),
                    calculate_cosine_similarity(
                        get_function_name(method_body), reduced_tokens
                    ),
                    calculate_cosine_similarity(
                        extract_variables_names(method_body), reduced_tokens
                    ),
                ]

                # Save the data to Excel
                save_to_excel(data, "CosineSimilarityAnalysis_noFN")
